...
See [`MdKitEditorProps`](./reference/index/type-aliases/MdKitEditorProps.md) for the full component
props.
...
- `--mp-lb-mdkit-code-block-padding`
...
`MdKitEditor` highlights fenced code blocks with TipTap's lowlight extension.
The default `codeTheme` is `auto`, using `github` in light mode and
`github-dark` in dark mode via `prefers-color-scheme`.

```tsx
<MdKitEditor codeTheme="darcula" value={markdown} onChange={setMarkdown} />
```

Use an explicit pair when your app controls light and dark themes:
...
Bundled theme names come from highlight.js CSS themes:
...
Set `syntaxHighlighting={false}` to keep plain code blocks. For custom language
sets, pass `syntaxHighlighting={{ lowlight }}` with your own lowlight instance.
...
Related exports are listed in the [API reference](./reference/index.md).
